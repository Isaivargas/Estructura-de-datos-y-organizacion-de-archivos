//
//  main.c
//  practica8recursividad
//
//  Created by Isaí Vargas on 10/30/17.
            ...
            case 3:printf("Introduce la opcion deseada para recorrer\t1 recorre simple \t2 recorre inversamente\n");
                ...
                switch (op) {
                    case 1:exito=recorre_simple(inicio);
                        break;
                    case 2:exito=printReverse(inicio);
                        break;
...
short int printReverse(struct nodo_simple *inicio)
{
    ...
    printf("%d  %s", inicio->data.num_jug,inicio->data.nombre_jug);
    return 1;
}
        ...
        if(!(*inicio)){
            *inicio=nuevo;
            ...
            if((*fin))
            {
                ...
                ins_fin_simple(&(*inicio),&(*fin)->next,origen);
            }
